...
def test_load_registry_pytorch_head_via_weights_uri(tmp_path):
    ...
    p = tmp_path / "registry.yaml"
    p.write_text(SAMPLE_PYTORCH_YAML)

    sentinel_session = object()
    with patch(
        "tongue_ai.pytorch_session.build_pytorch_session", return_value=sentinel_session
    ) as build_mock:
...
def test_load_registry_exposes_category_map(tmp_path):
    ...
    with patch("tongue_ai.pytorch_session.build_pytorch_session", return_value=object()):
        registry = load_registry(p)
